...
void chassisSetState(float vx, float vy, float targetAngle)
{
	...
	body_vx = vx * cosf(-targetAngle / 360 * (2 * 3.14159)) 
						- vy * sinf(-targetAngle / 360 * (2 * 3.14159));
	body_vy = vx * sinf(-targetAngle / 360 * (2 * 3.14159)) 
						+ vy * cosf(-targetAngle / 360 * (2 * 3.14159));
...
void chassisAngleRing(void)
{
	...
	float angle = chassis.angle;
	int numOfTurns = chassis.numOfTurns;
	...
	if(chassis.smoothTargetAngle <= chassis.targetAngle - 2.0)
	{
		chassis.smoothTargetAngle += 2.0;
	}
	else if(chassis.smoothTargetAngle >= chassis.targetAngle + 2.0)
	{
		chassis.smoothTargetAngle -= 2.0;
	}
	else
	...
	chassis.rotatingSpeed = -pidOutput(&(chassis.AngleRing_pid), chassis.smoothTargetAngle, angle + numOfTurns * 360.0);
